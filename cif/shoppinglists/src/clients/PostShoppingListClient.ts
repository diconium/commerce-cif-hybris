...
export default class PostShoppingListClient extends HttpClient {
  ...
  private modifyInputs(cartDto: CartWsDTO) {
    ...
    if (productVariantId) {
      const orderEntry: OrderEntryWsDTO = new ShoppingListEntryMapper(this.input.settings).mapFromEntity({ quantity, productVariantId });
